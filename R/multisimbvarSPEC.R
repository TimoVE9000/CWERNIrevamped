...
multisimbvarSPEC=function (tmax, b1, d1, k1, bneck,kneckstart, kneckend, abun_original, nsim, interval, wantedtimes){
  ...
  while(counter<=nsim){
    ...
    utils::write.csv(test, file = paste(counter, "simdata_simbvar.csv",sep=""))
    counter=counter+1
  ...
  while(timepointcounter<=length(wantedtimes)){ #Cycle through the timepoints for which you want plots (whole integers!)
    ...
    counter=1
    while(counter<=nsim){
      MyData <- utils::read.csv(file=paste(counter, "simdata_simbvar.csv",sep=""), header=TRUE, sep=",")
    ...
    grDevices::tiff(filename=paste(wantedtime,"time_rac_simbvar.tiff",sep=""), width = 1000, height = 1000, units = "px", pointsize = 32)
  ...
  #Trajectories
  grDevices::tiff(filename="Trajectory_totalcommsize_simbvar.tiff", width = 1000, height = 1000, units = "px", pointsize = 32)
  graphics::plot(1, type="n", xlab="time", ylab="Total community size", ylim=c(0,20000), xlim=c(0,tmax))

  counter=1 #cycle .csv files
  while(counter<=nsim){
    MyData <- utils::read.csv(file=paste(counter, "simdata_simbvar.csv",sep=""), header=TRUE, sep=",") #reading in .csv
    readdata=as.matrix(MyData)
    readdata=readdata[,2:ncol(readdata)]
    readdata=apply(readdata, 2, as.numeric)
