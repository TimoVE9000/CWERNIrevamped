...
#'
#' @examples
#' multinulsim(10, 0.6, 0.1, 16000,
#' generate_spat_abund(theta = 200,Ivec = rep(40,1),Jvec = c(16000)),
#' 2,"nulsim.tiff",  200)
#'
#'
#'
multinulsim=function (tmax, b1, d1, k1, abun_original ,nsim, filenomen, interval){
  ...
  while (count<nsim){ #Preform nsim number of simulations
    test=nulsim(tmax, b1, d1, k1, abun_original, interval) #do simulations
    rac=sort(test [(2):(nrow(test)),length (test[1,])],decreasing = T)#calculate RAC
    racstor=rbind (racstor, rac) #store RAC
    #points (rac)
    traj=cbind(test[1,], colSums(test [2:nrow(test), ])) #calculate community size trajectory over time
